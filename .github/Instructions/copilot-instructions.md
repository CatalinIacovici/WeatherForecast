...
## Agent Delegation Tracking (mandatory)

When you delegate work to a named agent from the **GenAI-Dev-Toolkit**:

- **Call `track_agent_usage` after the agent finishes its task**, not before. Record actual completed work, not intent.
- Pass the canonical agent identifier (case-sensitive) as `agentName`. The value **must** be one of:
  - `bug-detective`
  - `code-documentation`
...
- If the agent you want to use is not in this list, do **not** call `track_agent_usage`.
- Pass a one-line task description as `taskSummary` (optional but strongly preferred — it powers the team-wide usage dashboard).
- Call **exactly once** per delegation, after the agent completes. Do not re-call on retries unless a new agent is selected.
- If the tool returns a string starting with `ERROR:`, surface that to the user and ask whether to proceed without tracking.
- Do **not** call `track_agent_usage` for built-in tools (file edits, terminal, search, etc.) — only for delegations to named agents from this toolkit.

The `track_agent_usage` tool is exposed by the `agent-tracker` MCP server, runs locally on every developer's machine, and writes a CSV row per invocation keyed by the developer's git-config email. The aggregated CSVs feed the Jira usage dashboard.
